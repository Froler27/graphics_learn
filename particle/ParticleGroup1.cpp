...
void CParticleGroup1::init()
{
	...
	particleTemplate.setMass(1.f);//设置重量

	osg::ref_ptr<osgParticle::ParticleSystem> rpPS = new osgParticle::ParticleSystem;//创建粒子系统
	rpPS->setDefaultAttributes("Images/smoke.rgb", true, false);//设置材质，是否放射粒子，是否添加光照
	...
	osg::ref_ptr<osgParticle::SectorPlacer> rpPlacer = new osgParticle::SectorPlacer;//设置一个点放置器
	rpPlacer->setCenter(osg::Vec3(0.f, 0.f, 0.f));//设置位置
	rpPlacer->setRadiusRange(2.5, 5);
	...
	rpShooter->setInitialSpeedRange(50, 1000);//设置发射器速度变化范围
